...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  _config_mode = "STANDARD";
  _velocity_mode = "MAXWELL";
...
void System :: finalize(){
  ...
  _rnd.SaveSeed(output_path("seed.out"));
  ofstream coutf;
  coutf.open(output_path("output.dat"),ios::app);
...
void System :: write_XYZ(int nconf){
  ofstream coutf;
  coutf.open(output_path("CONFIG/config_" + to_string(nconf) + ".xyz"));
  if(coutf.is_open()){
    coutf << _npart << endl;
    coutf << "#Comment!" << endl;
    for(int i=0; i<_npart; i++){
...
// Read configuration from a .xyz file in directory INPUT/<run>/CONFIG/
void System :: read_configuration(){
  ...
  if(_config_mode == "STANDARD"){
    config_file = "CONFIG/config.xyz";
    config_scale = 1.0;
  } 
  else if(_config_mode == "FCC_FULL_BOX"){
    config_file = "CONFIG/config.fcc";
    config_scale = 1.0;
  } 
  else if(_config_mode == "FCC_HALF_BOX"){
    config_file = "CONFIG/config.fcc";
    config_scale = 0.5;
  } 
  else {
  ...
  cinf.open(input_path(config_file));
  ...
  if(cinf.is_open()){
    ...
    if(ncoord != _npart){
      cerr << "PROBLEM: conflicting number of coordinates in input.dat and "
           << config_file << endl;
      exit(EXIT_FAILURE);
    }

    getline(cinf, comment);  // consume end of first line
    getline(cinf, comment);  // read xyz comment line

    for(int i=0; i<_npart; i++){

      cinf >> particle >> x >> y >> z; // coordinates are in units of box side
    ...
    cerr << "PROBLEM: Unable to open INPUT file " << input_path(config_file) << endl;
    exit(EXIT_FAILURE);
  }
  ...
  if(_restart and _sim_type > 1){
    ...
    int spin;
    cinf.open(input_path("CONFIG/config.spin"));
    ...
    if(cinf.is_open()){
      ...
      cerr << "PROBLEM: Unable to open INPUT file "
           << input_path("CONFIG/config.spin") << endl;
      exit(EXIT_FAILURE);
    }
  }
  ...
  if (_measure_pofv){
    for(int i=0; i<_npart; i++){
      ...
      if(bin_v >= 0 && bin_v < _n_bins_v){
        _measurement(_index_pofv + bin_v) += 1.0 / (double(_npart) * _bin_size_v);
      }
    }
  }
...
void System :: averages(int blk){
  ...
  // POFV //////////////////////////////////////////////////////////////////////
  if (_measure_pofv){
    coutf.open(output_path("pofv.dat"),ios::app);
    ...
    for(int i=0; i<_n_bins_v; i++){
      ...
      sum_average = _global_av(index);
      sum_ave2 = _global_av2(index);
    ...
    // pofv_blocks.dat: istogramma del singolo blocco, utile per 4.2
    ofstream coutfb;
