...
double Random :: Exponential(double lambda) {
   ...
   double u=Rannyu();
   double x=-(1.0/lambda)*log(1-u);
   return x;
}

double Random :: Cauchy(double mu, double gamma) {
   // This function generates a random number from an Cauchy distribution with given mu and lambda
   double u=Rannyu();
   double x=mu + gamma*tan(M_PI*(u-1.0/2.0));
   return x;
}
