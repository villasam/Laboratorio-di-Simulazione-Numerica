  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8.0 * M_PI * _rho * (1.0/(9.0*pow(_r_cut,9)) - 1.0/(3.0*pow(_r_cut,3))); //aggiunta 7
        ...
        index_property++;
        _ptail = 32.0 * M_PI * _rho * (1.0/(9.0*pow(_r_cut,9)) - 1.0/(6.0*pow(_r_cut,3)));; //aggiunta7
      } else if( property == "GOFR" ){ //modifica 7
        ofstream coutgr(output_path("gofr.dat"));
        coutgr << "#     BLOCK:     DISTANCE:    BLOCK_GOFR:    GOFR_AVE:      ERROR:" << endl;
        coutgr.close();
        input >> _n_bins;
        _nprop += _n_bins;
        ...
        coutf.close();
        break;
      } else cerr << "PROBLEM: unknown property: " << property << endl;
    }
    input.close();
...
void System :: write_configuration(){
  ofstream coutf;
  if(_sim_type < 2){
    coutf.open(output_path("CONFIG/config.xyz"));
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open CONFIG/config.xyz" << endl;
    coutf.close();
    coutf.open(output_path("CONFIG/conf-1.xyz"));
    if(coutf.is_open()){
      coutf << _npart << endl;
      coutf << "#Comment!" << endl;
      for(int i=0; i<_npart; i++){
    ...
    } else cerr << "PROBLEM: Unable to open CONFIG/conf-1.xyz" << endl;
    coutf.close();
  } else {
    coutf.open(output_path("CONFIG/config.spin"));
...
void System :: read_configuration(){ //corretta7
  ...
  if(_sim_type < 2){
    ...
    if(_config_mode == "FCC"){
      config_file = "../INPUT/CONFIG/config.fcc";
    } else if(_config_mode == "STANDARD"){
      config_file = input_path("CONFIG/config.xyz");
    } else {
      cerr << "PROBLEM: unknown CONFIG_MODE: " << _config_mode << endl;
      exit(EXIT_FAILURE);
    }

    cinf.open(config_file);
    ...
    if(cinf.is_open()){
      ...
      if (ncoord != _npart){
        cerr << "PROBLEM: conflicting number of coordinates in input.dat and configuration file" << endl;
        exit(EXIT_FAILURE);
      }
      ...
      cerr << "PROBLEM: Unable to open configuration file: " << config_file << endl;
      exit(EXIT_FAILURE);
    }
  }
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        // Radial distribution function g(r) aggiunta 7
        if(_measure_gofr && dr < _halfside.min()){
          bin = int(dr / _bin_size);

          if(bin >= 0 && bin < _n_bins){
            double r_lower = bin * _bin_size;
            double r_upper = (bin + 1) * _bin_size;

            double shell_volume = (4.0/3.0) * M_PI *(pow(r_upper,3) - pow(r_lower,3));

            _measurement(_index_gofr + bin) += 2.0 / (_rho * double(_npart) * shell_volume);
          }
        }
  ...
  // KINETIC ENERGY //////////////////////////////////////////////////////////// /aggiunta7
  bool need_kinetic =
      (_sim_type == 0) &&
      (_measure_kenergy || _measure_temp || _measure_tenergy || _measure_pressure);
  ...
  // TEMPERATURE /////////////////////////////////////////////////////////////// /aggiunta7
  if (_measure_temp) {
    if (_sim_type == 0) {
      _measurement(_index_temp) = (2.0/3.0) * kenergy_temp;
    } else if (_sim_type == 1) {
      _measurement(_index_temp) = _temp;
    }
  }
  // PRESSURE //////////////////////////////////////////////////////////////////
  if (_measure_pressure) {
    double ideal_pressure = 0.0;

    if (_sim_type == 0) {
      ideal_pressure = _rho * (2.0/3.0) * kenergy_temp;
    } else if (_sim_type == 1) {
      ideal_pressure = _rho * _temp;
    }

    _measurement(_index_pressure) = ideal_pressure + (_ptail*_npart + 48.0*virial/3.0)/_volume;
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
  if (_measure_magnet) {
    for(int i = 0; i < _npart; i++) {
...
void System :: averages(int blk){
  ...
  if (_measure_gofr) {
    ...
    ofstream coutgr(output_path("gofr.dat"), ios::app);

    for (int i = 0; i < _n_bins; i++) {

      int index = _index_gofr + i;
