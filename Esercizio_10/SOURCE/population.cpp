...
vector<int> Population::GetBestPath() const {

    return _individuals[0].GetPath();
}
...
void Population::ReplaceBestPath(const vector<int>& path, const Map& map) {

    _individuals[0].SetPath(path);
    _individuals[0].ComputeLength(map);

    Sort();
}

void Population::ReplaceWorstPath(const vector<int>& path, const Map& map) {

    _individuals.back().SetPath(path);
    _individuals.back().ComputeLength(map);

    if(!_individuals.back().Check()) {
        cerr << "ERROR: invalid migrant path" << endl;
        exit(EXIT_FAILURE);
    }
